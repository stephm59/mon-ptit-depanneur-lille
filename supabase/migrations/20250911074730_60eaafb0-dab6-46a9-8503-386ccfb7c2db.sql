-- Update bathroom renovation blog post
UPDATE blog_posts 
SET 
  title = 'Rénover sa salle de bains : douche ou baignoire ? | Mon P''tit Dépanneur',
  content = 'Lorsqu''on rénove une salle de bains, le dilemme revient toujours : vaut-il mieux installer une douche moderne et pratique ou une baignoire élégante et relaxante ? La vérité, c''est qu''il n''y a pas de bon ou de mauvais choix. Tout dépend :
...
Ce bon conseil vous est proposé par Mon P''tit Dépanneur, artisan spécialisé en [rénovation de salle de bains à Lille](/renovation-salle-de-bains-lille).',
  updated_at = NOW()
